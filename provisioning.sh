...
# Debian-Pakete upgraden
echo "* Debian-Pakete upgraden..."
export DEBIAN_FRONTEND=noninteractive
apt-get update
apt-get --yes dist-upgrade

# Memcached konfigurieren
echo "* Memcached konfigurieren..."
...
# IP und Hostname werden in Vagrantfile als Argument übergeben
...
# Install Drupal 7: PROVISION_ARGS="install_drupal7" vagrant provision
if [[ $(echo "$@" | grep -e '\binstall_drupal7\b') ]]
then
  echo "* Install Drupal 7..."
  mysqladmin drop -f drupal
  mysqladmin create drupal
  chmod 755 /vagrant/drupal/sites/default/
  ...
  /home/vagrant/.composer/vendor/bin/drush dl drupal-7 --drupal-project-rename=drupal --destination=/vagrant
  cd /vagrant/drupal
...
# Install Drupal 7: PROVISION_ARGS="install_drupal7" vagrant provision
if [[ $(echo "$@" | grep -e '\binstall_drupal8\b') ]]
then
  echo "* Install Drupal 8..."
  mysqladmin drop -f drupal
  mysqladmin create drupal
  chmod 755 /vagrant/drupal/sites/default/
  rm -rf /vagrant/drupal
  mysql -e "GRANT SELECT, INSERT, UPDATE, DELETE, CREATE, DROP, INDEX, ALTER ON drupal.* TO 'vagrant'@'192.168.50.1' IDENTIFIED BY 'vagrant'"
  mysql -e "GRANT SELECT, INSERT, UPDATE, DELETE, CREATE, DROP, INDEX, ALTER ON drupal.* TO 'vagrant'@'localhost' IDENTIFIED BY 'vagrant';"
  /home/vagrant/.composer/vendor/bin/drush dl drupal-8 --drupal-project-rename=drupal --destination=/vagrant
  cd /vagrant/drupal
  /home/vagrant/.composer/vendor/bin/drush site-install standard --site-name=Drugrant --account-name=admin --account-pass=admin --db-url=mysql://vagrant:vagrant@localhost/drupal -y
fi
